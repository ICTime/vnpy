...
tickFieldMap[9] = 'lastPrice'
# http://xavierib.github.io/twsapidocs/classIBApi_1_1TickType.html  9 acturally CLOSE price 
...
class IbGateway(VtGateway):
    ...
    def __init__(self, eventEngine, gatewayName='IB'):
        ...
        print "-------------ibGateway.ibGateway.init" 
        ...
        self.subscribeVtSymbolList= [] # 每个VtSymbol只subscribe一次
    ...
    def subscribe(self, subscribeReq):
        ...
        contract.m_exchange = str(subscribeReq.exchange)
        contract.m_secType  = str(subscribeReq.productClass)
        contract.m_currency = str(subscribeReq.currency)
        contract.m_expiry   = str(subscribeReq.expiry)
        # contract.m_strike = subscribeReq.strikePrice
        # contract.m_right = optionTypeMap.get(subscribeReq.optionType, '')
    

	if not subscribeReq.vtSymbol in self.subscribeVtSymbolList:  
	    print "ibGateway.subscribe: --- ",contract.m_symbol, contract.m_exchange, contract.m_secType, contract.m_currency, contract.m_expiry , self.tickerId 
            self.connection.reqMktData(self.tickerId, contract, '', False)
	    self.subscribeVtSymbolList.append(subscribeReq.vtSymbol)
        ...
        tick.vtSymbol =  subscribeReq.vtSymbol
    ...
    def sendOrder(self, orderReq):
        ...
        contract.m_exchange = str(orderReq.exchange)
        contract.m_secType = str(orderReq.productClass)
        contract.m_currency = str(orderReq.currency)
    ...
    #----------------------------------------------------------------------
    def getHistBars(self,subscribeReq, endDateTime, durationStr, barSizeSetting, whatToShow):
        self.tickerId += 1
        """主查历史数据"""
	print "ibGateway.getBars tickerId --- ", self.tickerId 
        contract = Contract()
        contract.m_symbol = str(subscribeReq.symbol)
        contract.m_exchange = str(subscribeReq.exchange)
        contract.m_secType = str(subscribeReq.productClass)
        ...
        self.connection.reqHistoricalData(self.tickerId, contract, endDateTime, durationStr, barSizeSetting, whatToShow, 0, 1)
        log = VtLogData()
        log.gatewayName = self.gatewayName        
        self.onLog(log) 

    def getRealBars(self,subscribeReq,barSize,whatToShow):
        self.tickerId += 1
	print "ibGateway.getRealBarst --- "
        contract = Contract()
        contract.m_symbol = str(subscribeReq.symbol)
        contract.m_exchange = str(subscribeReq.exchange)
        contract.m_secType = str(subscribeReq.productClass)
        ...
        self.connection.reqRealTimeBars(self.tickerId, contract,barSize,whatToShow,0)
	print "ibGateway.getRealBars: --- ",contract.m_symbol, contract.m_exchange, contract.m_secType, contract.m_currency, contract.m_expiry , self.tickerId 
...
class IbWrapper(EWrapper):
    ...
    def __init__(self, gateway):
        ...
        # Load Gateway Static Contract list   	
        # fileName = self.gatewayName + '_contract.json'
        # fileName = os.getcwd() + '\\ibGateway\\' + fileName
        # self.gateway.loadGatewayContract(fileName)
    ...
    def tickPrice(self, tickerId, field, price, canAutoExecute):
        ...
        if field in tickFieldMap:
            ...
            print "ibGateway.tickPrice: else ", tickerId, field  
            # print field

    #----------------------------------------------------------------------
    def tickSize(self, tickerId, field, size):
        """行情推送（量相关）"""
        # print "ibGateway.tickSize: ", tickerId, field,size  
        if field in tickFieldMap:
            tick = self.tickDict[tickerId]
            key = tickFieldMap[field]
            tick.__setattr__(key, size)       
        else:
            print "ibGateway.tickSize: else", tickerId
            print field
    ...
    def tickString(self, tickerId, tickType, value):
	    ...
	    # TODO  %H:%M:%S FAIL 
            tick.time = strftime('%H:%M:%S', lt) + ".0"
            tick.date = strftime('%Y%m%d')
            # print "ibGateway.tickString: ", tickerId, value , tick.time
    ...
    def updatePortfolio(self, contract, position, marketPrice, marketValue, averageCost, unrealizedPNL, realizedPNL, accountName):
        ...
        pos.exchange = contract.m_exchange
    ...
    def execDetails(self, reqId, contract, execution):
        ...
        trade.exchange = contract.m_exchange
    ...
    def historicalData(self, reqId, date, open, high, low, close, volume, count, WAP, hasGaps):
        ...
        print date, open, high, low, close, volume, count 
    ...
    def realtimeBar(self, reqId, time, open, high, low, close, volume, wap, count):
        ...
        # pass
        # print reqId, strftime('%H:%M:%S', localtime(time)), open, high, low, close, volume
	# realtimeBar is 5 sec, to get an uniform model, we can output into ticks with uniq price
	tick = self.tickDict[reqId] 
        # tick.time = strftime('%H:%M:%S', localtime(time))
        tick.time = strftime('%H:%M:%S', localtime(time)) + ".0"
        tick.date = strftime('%Y%m%d')
        for price in set([open,high,low,close]):
